# Module name: connections/http_driver.py
...
class HttpDriver(GenericDriver):
    def load(self) -> None:
    ...
    def write(self, uri: str, filename: str, ftype: FileType, data: object, **kwargs) -> str:
        raise NotImplementedError(
            ...
            "Use LocalStorageDriver for local persistence, or implement "
    ...
    def _build_request_kwargs(self, **kwargs) -> dict:
        ...
        When ``connection_name`` is set on the driver the session already
        carries auth and proxy configuration; per-call kwargs still override.
        """
        request_kwargs: dict = {
    ...
    def _download(self, uri: str, **kwargs) -> requests.Response:
        ...
        connection: Optional[ProxyConnection] = self.connection_name
